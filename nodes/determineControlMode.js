        ...
        node.on("input", function (msg) {
            ...
            try {
                ...
                if (pvForecast < minPvRequired) {
                    if (debug) {
                        node.warn(`pvForecast (${pvForecast}) < minPvRequired (${minPvRequired})`);
                    }
                    msg.pvMIN = true;
                    if (Math.round(priceDifference * 10) > Math.round(minPriceDifference * 10)) {
                        if (debug) {
                            node.warn(`priceDifference (${priceDifference}) > minPriceDifference (${minPriceDifference})`);
                        }
                        msg.priceOK = true;
                        msg.priceDIFFERENCE = minPriceDifference;
                        if (pvForecast * 1.6 < minPvRequired) {
                            if (debug) {
                                node.warn(`pvForecast * 1.6 (${pvForecast * 1.6}) < minPvRequired (${minPvRequired})`);
                            }
                        ...
                        if (Math.round(priceDeviation * 10) > Math.round(minPriceDeviation * 10)) {
                            if (debug) {
                                node.warn(`priceDeviation (${priceDeviation}) > minPriceDeviation (${minPriceDeviation})`);
                            }
                            ...
                            if (Math.round(avgPrice * 100) > Math.round(batteryControlLimit * 100)) {
                                if (debug) {
                                    node.warn(`avgPrice (${avgPrice}) > batteryControlLimit (${batteryControlLimit})`);
                                }
