module.exports = function(RED) {
	function EstimateBatteryMode(config) {
		...
		node.on("input", function(msg) {
			...
			if (debug) { node.warn("Netzladung aktiviert (conf): " + charge); }
			...
			*/
			...
			const batteryModes = energyAvailable.map((hour) => {
				...
				if (debug) { node.warn("initialer Batterieleistung: " + startBatteryPower); }
				if (debug) { node.warn("aktueller Batterieleistung: " + currentbatteryPower); }
				if (debug) { node.warn("verbrauchte Batterieleistung: " + usedEnergy); }

				// Berechnung der verbleibenden Batterieleistung
				let remainingEnergy = Math.max(0, startBatteryPower - usedEnergy);
				if (debug) { node.warn("verbleibende batteryPower: " + remainingEnergy); }
				...
				estimatedbatteryPower = Math.max(minEnergy, estimatedbatteryPower + Math.min(battery_capacity + minEnergy, remainingEnergy + ((batteryCapacity / 100) * estimatedMaximumSoc.soc)));
				if (debug) {
			...
			for (let i = 0; i < batteryModes.length; i++) {
				if (batteryModes[i].mode == "charge" && batteryModes[i].energy !== "undefined") {
					...
					if (debug) { node.warn(i + ": Power: " + power + " / Grid: " + grid + " / Old: " + old + " / homePrice: " + energyPrice); }
					...
					if (batteryModes[i].mode == "hold" && batteryModes[i].value < 0) {
						...
						if (debug) { node.warn(i + ": Power: " + power + " / PV: " + pv + " / Old: " + old + " / homePrice: " + energyPrice); }
						estimatedbatteryPower = power;
					}
					...
					if (batteryModes[i].mode == "normal" && batteryModes[i].energy !== "undefined") {
						let power = Math.max(minEnergy, Math.min(battery_capacity + minEnergy, estimatedbatteryPower - Math.abs(batteryModes[i].energy)));
						estimatedbatteryPower = power;
						if (debug) { node.warn(i + ": Power: " + power + " / homePrice: " + energyPrice); }
					}
				}
				...
				if (batteryModes[i].mode == "charge") {
					batteryModes[i].cost2 = (batteryModes[i].importPrice * Math.abs(batteryModes[i].energy) * factor) + (batteryModes[i].value * batteryModes[i].importPrice);
				}
